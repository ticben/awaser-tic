...
const GalleryView: React.FC<GalleryViewProps> = ({ onSelectArtwork, lang }) => {
  ...
  return (
    <div className="h-full bg-slate-950 pt-32 pb-40 px-6 overflow-y-auto no-scrollbar">
        ...
        <div className="mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 snap-x">
            {recentArtworks.map(artwork => (
              <div 
                key={`recent-${artwork.id}`}
                onClick={() => onSelectArtwork(artwork)}
                className="flex-shrink-0 w-44 snap-start cursor-pointer active:scale-95 transition-transform"
              >
                <div className="glass-morphism rounded-[28px] overflow-hidden border border-white/10 shadow-lg relative aspect-[4/5]">
                  ...
                  <div className="absolute top-3 right-3 p-1.5 bg-indigo-600/80 rounded-full text-white shadow-lg">
                    <Sparkles size={10} />
                  </div>
                </div>
              </div>
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-xs font-medium leading-relaxed">Archived and active virtual installations across the city.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {ARTWORKS.map(artwork => (
          <div 
            key={artwork.id} 
            onClick={() => onSelectArtwork(artwork)}
            className="group glass-morphism rounded-[40px] overflow-hidden cursor-pointer transform transition-all active:scale-95 border border-white/5 shadow-2xl"
          >
            <div className="h-56 relative overflow-hidden">
              ...
              <div className="absolute top-6 left-6 bg-indigo-600/90 backdrop-blur-md px-4 py-1.5 rounded-full text-[9px] font-black text-white uppercase tracking-widest shadow-lg">
                {artwork.category}
              </div>
            ...
            <div className="p-8">
              <div className="flex justify-between items-start mb-2">
                 ...
                 <div className="p-2 bg-white/5 rounded-xl text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
                    <History size={16} />
                 </div>
              </div>
              <p className="text-indigo-400 text-[10px] font-black uppercase tracking-widest mb-4">@{artwork.artist}</p>
              <p className="text-slate-400 text-xs line-clamp-2 leading-relaxed font-medium mb-6">
                {artwork.description}
              </p>
